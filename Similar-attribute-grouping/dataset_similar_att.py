...
class MuSeSimAttDataset(Dataset):
    def __init__(self, data, partition, label_dims):
        super(MuSeSimAttDataset, self).__init__()
    ...
    def __getitem__(self, idx):
        ...
        label = {label_dim: label[idx] for label_dim, label in self.labels.items()}
        meta = {label_dim: meta[idx] for label_dim, meta in self.metas.items()}

        sample = feature, feature_len, label, meta
        return sample
    
def custom_sim_att_collate_fn(data):
    """
